...
- **2026-05-22** — auditoria paranoica completa (2 passadas) aplicando o framework [Protocolo de Segurança](https://github.com/jeanderson-silva8/protocolo-de-seguranca) — [relatório](docs/AUDIT_REPORT_2026-05-22.md). Resultado: 0 vulnerabilidades críticas exploráveis, 1 bug funcional crítico corrigido (enum de `Task`), endurecimentos de auth aplicados, suíte de 33 testes adversariais (`node:test` + Supertest + `mongodb-memory-server`) e CI no GitHub Actions cobrindo testes + audit + guard de segredo no bundle.
- **2026-05-22 (peer review)** — terceira passada por revisor externo identificou 3 achados que escaparam: (a) `ObjectId.isValid` ausente no `reset-password/:id/:token` — correção inconsistente em relação a `tasks.js`; (b) `reset-password` sem testes adversariais dedicados; (c) `credentials: true` no CORS sem uso de cookie. Todos corrigidos no mesmo dia. Os achados (a) e (b) geraram os **itens 59 e 60** novos no framework Protocolo de Segurança v1.9.0.
